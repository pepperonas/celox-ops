...
- **Unit-Tests — 176 gesamt** (alle DB-frei, laufen in CI bei jedem Push):
  ...
  - **Frontend (Vitest, 69):** `formatters` (14), `validators` (9), `decimal` (6 — Komma/Punkt-Parsing), `positions` (5 — Auto-Positions-Erkennung), `AutocompleteInput` (13 — Positions-/Titel-/Rabatt-Vorschlagspools: Umfang, Dubletten, Themenabdeckung), Rainmaker-`constants` (5), `dreamPresets` (9 — Traumziel-Presets/Motivations-Mathe), `exchangeRate` (3 — Kurs-Plausibilität), `chartTooltip` (5 — Umsatz-Tooltip)

## Projektumfang

- **~31.700 LoC Anwendungscode** — ~11.500 Backend (Python/FastAPI) · ~1.620 Jinja-PDF-Templates · ~18.600 Frontend (TypeScript/React)
- **~2.350 LoC Tests** · 22 DB-Tabellen · 176 Unit-Tests · Mehrbenutzer mit isolierten Arbeitsbereichen
