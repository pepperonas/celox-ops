...
- **Unit tests — 110 total** (all DB-free, run in CI on every push):
  - **Backend (pytest, 67):** `test_smoke` (8), `test_invoice_service` (12 — totals/discounts/rounding), `test_auth` (6 — JWT), `test_rainmaker` (19 — activation engine/streak/points), `test_compliance` (6 — required-doc engine), `test_github_summary` (11 — commit grouping C1), `test_dashboard` (5 — sargable month bounds B5)
...
- **~900 LoC tests** · 22 DB tables · 110 unit tests · multi-user with isolated workspaces
