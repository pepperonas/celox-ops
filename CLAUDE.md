...
- **Tests (162 total, all DB-free)**: `backend/tests/` (pytest, **98**): `test_smoke` (config/router-import/invoice calc), `test_invoice_service` (totals + discounts + rounding), `test_auth` (JWT roundtrip/expiry/tamper), `test_rainmaker` (activation engine: next-action, rotting, working-day streak + freeze, points), `test_compliance` (`_build_items` signed/unsigned + `COMPLIANCE_REQUIRED_DEFAULTS`), `test_github_summary` (`_commit_theme` grouping/noise-folding — C1), `test_dashboard` (`_month_bounds` sargable ranges — B5), `test_rainmaker_dream` (Traumziel EV engine: ev-per-contact, weighted activity EV, pace projection), `test_invoice_discount_clear` (discount-clear recomputes totals — B-hunt regression), `test_exchange_rate` (Frankfurter parsing + plausibility bounds), `test_linkedin_import` (export parsers: Connections/Invitations/messages/ZIP-archive incl. guards). `frontend/src/**/*.test.ts` (Vitest, **64**): formatters, validators, `decimal` (comma/dot `parseDecimalInput`), `positions` (`isAutoPosition` auto/manual detection), `AutocompleteInput` (POSITION_SUGGESTIONS 706 + TITLE_SUGGESTIONS 491 + DISCOUNT_REASON_SUGGESTIONS 275: count/dedup/themes incl. celox portfolio), Rainmaker constants, `dreamPresets` (Traumziel presets + motivation math), `exchangeRate` (`sanitizeRate` plausibility). **CI runs both on every push** (backend `pytest`, frontend `vitest`). The dev backend container mounts only `app/`, so to run new test files in it, mount `tests/` (see Tests section) or rebuild. **Roughly ~31,700 LoC app code** (~11,500 backend Python · ~1,620 Jinja templates · ~18,600 frontend TS/TSX; ~2,350 LoC tests).
