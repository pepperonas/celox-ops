...
- **LinkedIn-Import** (Pipeline header button → `LinkedInImportModal`): imports LinkedIn connections as leads via LinkedIn's **official free GDPR data export** (Connections.csv — no API, no paid tools). Parser `services/linkedin_import.py` handles the "Notes:" preamble before the header, EN **and** DE column names ("First Name"/"Vorname" …), BOM, quoted commas; contacts without a company get the person name as `company` (NOT NULL). Endpoints: `POST /api/rainmaker/import/linkedin/preview` (multipart upload → parsed rows + `duplicate` flags) and `POST /api/rainmaker/import/linkedin` (selected rows → leads; re-checks duplicates server-side). Dedup is **per owner** (tenancy-scoped) by normalized profile URL (protocol/www stripped) or contact name, plus in-file dupes. Mapping: `source="LinkedIn-Import"`, `tags=["linkedin"]`, `website`=profile URL, `notes`="LinkedIn-Kontakt seit <Connected On>". Limits: 10 MB / 20k rows. Emails are mostly empty in the export (contacts must opt in).
...
- **Tests (156 total, all DB-free)**: `backend/tests/` (pytest, **92**): `test_smoke` (config/router-import/invoice calc), `test_invoice_service` (totals + discounts + rounding), `test_auth` (JWT roundtrip/expiry/tamper), `test_rainmaker` (activation engine: next-action, rotting, working-day streak + freeze, points), `test_compliance` (`_build_items` signed/unsigned + `COMPLIANCE_REQUIRED_DEFAULTS`), `test_github_summary` (`_commit_theme` grouping/noise-folding — C1), `test_dashboard` (`_month_bounds` sargable ranges — B5), `test_rainmaker_dream` (Traumziel EV engine: ev-per-contact, weighted activity EV, pace projection), `test_invoice_discount_clear` (discount-clear recomputes totals — B-hunt regression), `test_exchange_rate` (Frankfurter parsing + plausibility bounds), `test_linkedin_import` (Connections.csv parser: preamble/DE+EN headers/BOM/lead mapping). `frontend/src/**/*.test.ts` (Vitest, **64**): formatters, validators, `decimal` (comma/dot `parseDecimalInput`), `positions` (`isAutoPosition` auto/manual detection), `AutocompleteInput` (POSITION_SUGGESTIONS 706 + TITLE_SUGGESTIONS 491 + DISCOUNT_REASON_SUGGESTIONS 275: count/dedup/themes incl. celox portfolio), Rainmaker constants, `dreamPresets` (Traumziel presets + motivation math), `exchangeRate` (`sanitizeRate` plausibility). **CI runs both on every push** (backend `pytest`, frontend `vitest`). The dev backend container mounts only `app/`, so to run new test files in it, mount `tests/` (see Tests section) or rebuild. **Roughly ~27,400 LoC app code** (~9,840 backend Python · ~1,620 Jinja templates · ~15,900 frontend TS/TSX).
