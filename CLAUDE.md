...
- **Globales Undo-Muster**: `utils/undoToast.tsx` → `toastWithUndo(message, onUndo)` — Erfolgs-Toast mit „Rückgängig"-Button (8 s Fenster). Eingesetzt bei: Rechnungs-Statuswechsel (InvoiceDetail — dort auch ein freies „Status ändern"-Dropdown mit allen 5 Status; das Backend-`PUT /status` akzeptierte schon immer alle), Auftrags-Kanban-Drags, Rainmaker-Pipeline-Drags, und Löschungen mit **Wiederherstellen-durch-Neuanlage** (Ausgaben via ExpenseForm, Zeiteinträge, Kunden-Aktivitäten, *geplante* Rainmaker-Aktionen — *erledigte* bewusst nicht, sie tragen Punkte/Streak). Wiederhergestellte Objekte bekommen eine **neue ID**. Bei neuen Features mit umkehrbaren Aktionen dieses Muster verwenden statt `toast.success`.
