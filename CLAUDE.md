...
- **ServiceWorker (PWA)**: `frontend/public/sw.js` — **navigations (HTML) network-first** (always newest bundle refs; offline → cached `/index.html` then `/offline.html`); **hashed `/assets/*` cache-first** (immutable, offline-capable); `/api/*` never intercepted. Precaches the shell (`offline.html`, manifest, icons) on install. Cache name `celox-ops-v5` — **bump `CACHE_VERSION` on any app-shell change** to purge old caches. PWA icons: `public/icon-{192,512}.png` (`any`) + `icon-maskable-{192,512}.png` (`maskable`, generated from `icon-maskable.svg`) + `apple-touch-icon.png` (180, iOS — SVG isn't supported there); regenerate via ImageMagick (`magick -background "#0d1117" -density 900 favicon.svg -flatten -resize NxN out.png`). Mobile CSS lives in the "Mobile & touch ergonomics" block in `index.css` (16px inputs to stop iOS zoom, `touch-action: manipulation`, tap-highlight reset, 44px targets). Route pages are `React.lazy` (App.tsx) and Chart.js registration sits in the chart pages (`import '../utils/charts'`) so it code-splits out of the initial bundle.
