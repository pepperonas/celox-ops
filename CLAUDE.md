...
- **Motion / navigation craft** (`utils/transitions.ts`): in-app navigation runs through `useAppNavigate()`, which wraps the route change in a directional **View Transition** (`runViewTransition`, `data-nav="forward"|"back"`). Only `<main>` carries `view-transition-name: page` so the chrome (sidebar/header/QuickSearch) persists; pass `{ back: true }` for up-a-level moves. **Signature shared-element**: a clicked Rainmaker pipeline card gets `view-transition-name: rm-hero` and morphs into the lead-detail title (same name). `useScrollRestoration(mainRef)` remembers the `<main>` scrollTop per history entry and restores it on POP (no animated scroll); it also sets `html[data-pop="1"]` so entrance animations (`.md-stagger`, `animate-md-*`) don't replay on back/forward. Reactive `TiltCard` (Dashboard KPIs) tilts toward the cursor, gated to `(hover:hover) and (pointer:fine)` + reduced-motion. The Rainmaker "Erledigt" moment uses `.rm-complete-exit` (queue item) + `.rm-ring-pop` (progress ring). Fallback: no View-Transitions API or reduced-motion → instant navigation (progressive enhancement). The scroll container is `<main>`, NOT the window — restore against that element.
